...
void create_state_array(unsigned long long (*state_arr)[5][5], const unsigned char *m) {
    ...
    for (y = 0; y < 5; y++) {
        for (x = 0; x < 5; x++) {
            ...
            for (z = 0; z < w; z++) {
                i = w * (5 * y + x) + z;
                lane += ROL64((unsigned long long) m[i], z*8); // chars in m are 8-bit chunks. Rotate char to its correct position in the 64-bit unsigned long long 
            }
            (*state_arr)[x][y] = lane;
...
void chi(unsigned long long (*state_arr)[5][5]) {
    ...
    unsigned long long long_1 = 1; // Create 64 bit 1 for masking purposes
